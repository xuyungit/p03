#!/usr/bin/env python3
"""Optimized multi-case parameter fitting for time-series bridge data.

Performance improvements over fit_multi_case.py:
1. Pre-assemble structural system to avoid repeated LU factorization
2. Reuse system when structural parameters don't change
3. Cache system assembly for better performance with large datasets
...
class OptimizedMultiCaseFitter:
    """Optimized joint fitting with structural system caching."""
    ...
    def _get_or_assemble_system(self, struct_params: StructuralParams) -> StructuralSystem:
        """Get cached system or assemble new one if parameters changed."""
        # Create a hashable key from structural parameters
        param_key = (
        ...
        # Check if we can reuse cached system
    ...
    def _residual(
        ...
        """Compute stacked residuals with optimized forward evaluation."""
        self._n_residual_calls += 1
        
        struct_params, thermal_states = self._unpack_params(x, fit_struct)
        
        # Get (possibly cached) structural system
        system = self._get_or_assemble_system(struct_params)
        
        # 1. Physics residuals - batch compute all cases
        physics_residuals = []
    ...
    def fit(
        ...
        n_spatial = self.n_cases * max(0, self.temp_segments - 1) if self.temp_spatial_weight > 0 else 0
        n_temporal = (self.n_cases - 1) * self.temp_segments if self.temp_temporal_weight > 0 else 0
        n_regularization = n_spatial + n_temporal
...
def main():
    parser = argparse.ArgumentParser(
        description="优化版多工况联合拟合：固定结构参数 + 变化温度梯度"
    )
    parser.add_argument(
        '--data',
        type=Path,
    ...
    if args.use_true_temps:
        ...
        class FixedTempFitter(OptimizedMultiCaseFitter):
        ...
        # Create fitter
        fitter = OptimizedMultiCaseFitter(
            reactions_matrix=reactions_matrix,
            uniform_load=uniform_load,
            temp_spatial_weight=args.temp_spatial_weight,
            temp_temporal_weight=args.temp_temporal_weight,
