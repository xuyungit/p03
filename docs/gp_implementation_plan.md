...
- 对比与报告
  - [x] 与 NN/Tree 公平对比（统一列/增强/标准化）
    - 首轮对比：NN `experiments/bridge_nn/2025-09-30_10-59-59` vs SVGP `experiments/svgp_baseline/2025-09-30_13-44-49`，统一列正则 `^F[1-6]$、^N[0-9]+_r$、T_grad` / `^R[0-9]+$、^Ry_t[0-9]+$、^Rx_t[0-9]+$、D_t[0-9]+_r$`。SVGP 总体 MAE 3.71 vs NN 5.57，RMSE 15.87 vs 21.04，R² 0.9989 vs 0.9977，MAPE 2.41 vs 5.11。
    - 不确定性：SVGP 验证标定 τ≈0.408；测试集 raw NLL -2.39，0.9/0.95 覆盖率 0.989/0.995。全局 τ 缩放后（0.408）NLL -2.64，0.9 覆盖率降至 0.856；改用按输出族匹配 0.9 覆盖的分组 τ（R=0.496、Ry=0.476、Rx=0.460、D=0.445）可将测试集 0.9 覆盖率拉回 0.887，NLL 进一步提升到 -2.68 (`experiments/svgp_baseline/2025-09-30_13-44-49/per_group_tau.json`)。
  - [x] 统计显著性（≥3 个随机种子）
    - NN：seed 42/7/1337，MAE 5.72±0.12，RMSE 21.65±0.45。
